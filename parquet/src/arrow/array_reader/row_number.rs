...
impl RowNumberReader {
    pub(crate) fn new<I>(row_groups: impl IntoIterator<Item = I>) -> Self
    where
        I: Into<RowGroupSize>,
    {
        let row_groups = RowGroupSizeIterator::new(row_groups);
        Self {
            row_numbers: Vec::new(),
            row_groups,
        }
...
impl RowGroupSizeIterator {
    fn new<I>(row_groups: impl IntoIterator<Item = I>) -> Self
    where
        I: Into<RowGroupSize>,
    {
        Self {
            row_groups: VecDeque::from_iter(row_groups.into_iter().map(Into::into)),
        }
    }
}
...
impl From<&RowGroupMetaData> for RowGroupSize {
    fn from(rg: &RowGroupMetaData) -> Self {
        Self {
            first_row_number: rg.first_row_number(),
